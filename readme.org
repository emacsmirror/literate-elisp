...
- [[#tutorial][Tutorial]]
  ...
  - [[#lexical-binding-for-elisp-code-blocks][lexical binding for elisp code blocks]]
...
** lexical binding for elisp code blocks
By default ~literate-elisp-load~ evaluates code blocks with *dynamic* binding,
because Emacs's standard first-line cookie parser only accepts ~;; -*- ... -*-~
or ~#! ... ; -*- ... -*-~ — not the ~# -*- mode: org -*-~ used by org files.

The most common symptom: a lambda inside ~run-at-time~,
~set-process-filter~, or ~set-process-sentinel~ closes over a ~let~-bound
variable, and when it fires later Emacs signals ~void-variable~.

To opt into *lexical* binding for a single org file, declare it on the very
first line, alongside the org mode marker:
#+BEGIN_EXAMPLE
# -*- mode: org; lexical-binding: t; -*-
#+END_EXAMPLE
That cookie is detected by ~literate-elisp~ at load time on any Emacs
that exposes ~internal--get-default-lexical-binding-function~ — that
hook lives in master / future Emacs 31, no released 30.x has it yet.
Older Emacs are unaffected: a ~boundp~ guard makes the cookie support
a no-op, and org files keep their legacy dynamic-binding behavior.

#+BEGIN_SRC elisp :load no
;; Same code, two different outcomes depending on first-line cookie:
(defun example-closure ()
  (let ((counter 42))
    (run-at-time 0 nil
                 (lambda () counter))))   ; lexical: 42 ; dynamic: void-variable
#+END_SRC

Regression tests live at ~tests/test-lexical-binding.org~ (three repros that
fail without the cookie) and ~tests/test-no-cookie.org~ (no-cookie file
