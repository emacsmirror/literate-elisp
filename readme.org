...
#+END_EXAMPLE
That cookie is detected by ~literate-elisp~ at load time (Emacs 30+,
via ~internal--get-default-lexical-binding-function~).  Org files without
the cookie keep the legacy dynamic-binding behavior, so no existing file
is affected.
